...
class Driver:
    ...
    def login_corsalis_google(self, username, password):
        self.browser.get('https://corsalis.app/login')

        try:
            WebDriverWait(self.browser, self.wait_time).until(
                EC.element_to_be_clickable((By.ID, 'email'))
            )
            buttons_login_page = self.browser.find_elements(By.TAG_NAME, "button")
            button_google_login = buttons_login_page[2]
